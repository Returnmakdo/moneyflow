...
**톤 — 토스/배민 공지처럼 (절대 커밋로그처럼 X)**:
- 사용자 입장 + 혜택 중심으로. "내가 뭘 할 수 있게 됐는지"가 보여야 함.
- 친근체 (~해드려요 / ~할 수 있어요 / ~돼요 / ~이에요).
- 기술 용어 X — `EmptyCard`, `headerRowIndex`, `BottomSheet`, `state`, `prefs` 같은 단어 절대 X.
- 화면/필드 내부 명칭도 가급적 일반 표현으로. 예: "_subList의 EmptyCard에 도움말 진입점 추가" → "처음 들어온 화면에서 도움말 더 잘 보이게".
- title은 짧은 헤드라인. 예: "한 달치 지출, 명세서로 한 번에" / "밤에도 편하게, 다크모드". "~ 추가" / "~ 개선" 같은 명사형 헤드라인 X.
- items는 한 줄에 하나의 변화. "어떻게 구현했는지"가 아닌 "사용자가 뭘 보게 됐는지".

좋은 예: "탭을 다시 누르면 화면 맨 위로 부드럽게 올라가요"
나쁜 예: "ShellTabSignals에 5개 탭 ValueNotifier 확장 + ScrollController 연결"

좋은 예: "취소된 결제는 자동으로 빼서 명세서 합계랑 똑같이 맞춰드려요"
나쁜 예: "statusCol + excludedStatuses 추가로 취소 row 자동 필터링"
