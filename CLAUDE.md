...
> ⚠️ **출시 전(현재)에는 changelog entry를 추가하지 않는다.** 정식 출시 *이후*부터 사용자 체감 변경에 대해 갱신 시작. 출시 전 작업은 커밋만 하고 changelog는 건드리지 않음 — 출시 시점에 첫 entry부터 정리. 아래 워크플로는 *출시 이후* 기준.

`lib/data/changelog.dart`의 `changelog` 리스트가 설정 → "업데이트 소식" 화면에 그대로 노출. (출시 이후) **사용자가 체감하는 변경**이 있으면 반드시 맨 위에 entry 추가.
